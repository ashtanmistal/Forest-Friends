...
if os.path.exists(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "ubcv_campus_trees_processed.csv")):
    df = pd.read_csv(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "ubcv_campus_trees_processed.csv"))
else:
    # read the original csv file
    csv_file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "ubcv_campus_trees.csv")
    ...
    df.to_csv(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "ubcv_campus_trees_processed.csv"), index=False)
    ...
    if pd.isna(row["TREE_TYPE"]):
        if pd.isna(row["TAXA"]):
            if not pd.isna(row["COMMON_NAME"]):
                ...
                raise ValueError("Both the common name and taxon are missing for this row - this should have been filtered out earlier")
